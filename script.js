...
class MeowdiumApp {
    constructor() {
        ...
        this.selectedOverlay = 'cat1';
        this.overlayPosition = { x: 50, y: 20 };
        this.overlaySize = 80;
    ...
    setupCatifyMe() {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                this.currentPhoto = processedImage;
        ...
        overlayOptions.forEach(option => {
            option.addEventListener('click', () => {
                ...
                this.selectedOverlay = option.dataset.overlay;
                this.updateCatifyPreview();
            });
        });
        ...
        controlBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                switch (action) {
                    case 'up':
                        this.overlayPosition.y = Math.max(0, this.overlayPosition.y - step);
                        break;
                    case 'down':
                        this.overlayPosition.y = Math.min(80, this.overlayPosition.y + step);
                        break;
                    case 'left':
                        this.overlayPosition.x = Math.max(0, this.overlayPosition.x - step);
                        break;
                    case 'right':
                        this.overlayPosition.x = Math.min(80, this.overlayPosition.x + step);
                        break;
                    case 'rotate':
                        this.overlayRotation = (this.overlayRotation + 15) % 360;
                        break;
                    case 'bigger':
                        this.overlaySize = Math.min(150, this.overlaySize + 10);
                        break;
                    case 'smaller':
                        this.overlaySize = Math.max(30, this.overlaySize - 10);
                        break;
                    case 'reset':
                        this.overlayPosition = { x: 50, y: 20 };
                        this.overlaySize = 80;
    ...
    updateCatifyPreview() {
        ...
        preview.innerHTML = `
            <div style="position: relative; display: inline-block;">
                <img src="${this.currentPhoto}" style="max-width: 100%; height: auto; border-radius: 0.5rem;" />
                <div class="cat-overlay" style="
                    position: absolute;
                    left: ${this.overlayPosition.x}%;
                    top: ${this.overlayPosition.y}%;
                    width: ${this.overlaySize}px;
                    height: ${this.overlaySize}px;
                    background-color: ${overlayColors[this.getSelectedOverlay()]};
                    transform: rotate(${this.overlayRotation}deg);
                    font-size: ${this.overlaySize * 0.6}px;
                ">
    ...
    downloadCatifiedImage() {
        ...
        img.onload = () => {
            ...
            // Draw overlay
            const overlayX = (this.overlayPosition.x / 100) * img.width;
            const overlayY = (this.overlayPosition.y / 100) * img.height;
            const overlayW = (this.overlaySize / 100) * Math.min(img.width, img.height) * 0.3;
            
            ctx.save();
            ctx.translate(overlayX + overlayW/2, overlayY + overlayW/2);
            ctx.rotate((this.overlayRotation * Math.PI) / 180);
